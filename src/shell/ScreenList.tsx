...
import {formatStepNumber, stepReference, stepSummary, folderStepIds, isFallbackStep, folderDeletionReason} from './core';
...
export function ScreenList(props: ScreenListProps) {
  ...
  return <div class="folder-sidebar">
    <nav class="folder-rail" aria-label="Папки бота">
      <div class="folder-rail-list"><For each={props.document.folderOrder}>{id => <button type="button" class="folder-tab" classList={{selected: folderId() === id}} data-testid={`folder-tab-${id}`}
    ...
    <For each={screens()}>{id => {
      ...
      return <div data-testid={`step-nav-${id}`} data-screen-id={id} data-screen-sortable={!fallback() ? 'true' : undefined} class="screen-chat-row" classList={{'is-folder-fallback': fallback(), 'is-dragging': screenDrag.dragging() === id, 'is-link-target': props.linkTargetStepId === id}}>
        ...
        <button class="screen-chat" classList={{selected: props.selectedStepId === id}} aria-label={`Экран ${reference()}`} aria-description={!fallback() ? 'Нажмите, чтобы открыть. Перетащите для сортировки или на кнопку, чтобы назначить переход.' : undefined} aria-current={props.selectedStepId === id ? 'page' : undefined} onPointerDown={event => {if(!fallback()) screenDrag.begin(id, event);}} onClick={() => {if(screenDrag.allowClick()) props.onSelect(id);}}>
        <span class="screen-avatar" classList={{'is-entry': summary().isEntry, 'is-terminal': summary().isTerminal && !fallback(), 'is-fallback': fallback()}} aria-hidden="true">{formatStepNumber(props.document.steps[id].number)}</span>
