...
class TLDeserialization<FetchLongAs extends Long> {
  ...
  public fetchRawBytes(len: number | false, typed: true, field: string): Uint8Array<ArrayBuffer>;
  public fetchRawBytes(len: number | false, typed: false, field: string): number[];
  public fetchRawBytes(len: number | false, typed: boolean, field: string): Uint8Array<ArrayBuffer> | number[] {
    if(len === false) {
  ...
  public fetchObject(type: string, field?: string): any {
    ...
    if(type.charAt(0) === '%') {
      ...
      if(constructorCmp === gzipPacked) { // Gzip packed
        ...
        const uncompressed = gzipUncompress(compressed as unknown as ArrayBuffer) as Uint8Array<ArrayBuffer>;
