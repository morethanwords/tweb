...
import {LovelyChartPlotSize, LovelyChartRange, LovelyChartState, LovelyChartYRanges, StatisticsGraph, StatisticsGraphDataset} from './types';

export function createStateManager(data: StatisticsGraph, viewportSize: LovelyChartPlotSize, callback: (state: LovelyChartState) => void) {
  ...
  function update({range = {}, filter = {}, focusOn, minimapDelta}: Partial<LovelyChartState> = {}, noTransition?: boolean) {
  ...
  function _buildTransitionConfig() {
    ...
    const transitionConfig = mergeArrays([
      ANIMATE_PROPS,
      datasetVisibilities
    ]).map((transition) => {
      const [prop, duration, ...options] = transition.split(' ');
      return {prop, duration, options};
    });
...
function calculateState(
  ...
  viewportSize: LovelyChartPlotSize,
  range: LovelyChartRange,
  filter: LovelyChartState['filter'],
  focusOn: LovelyChartState['focusOn'],
  minimapDelta: LovelyChartState['minimapDelta'],
  prevState: LovelyChartState
) {
...
function calculateYRanges(data: StatisticsGraph, filter: LovelyChartState['filter'], labelFromIndex: number, labelToIndex: number, prevState: LovelyChartState): LovelyChartYRanges {
...
function calculateYRangesForGroup(data: StatisticsGraph, labelFromIndex: number, labelToIndex: number, prevState: LovelyChartState, datasets: StatisticsGraphDataset[]): LovelyChartYRanges {
...
function calculateYRangesStacked(data: StatisticsGraph, filter: LovelyChartState['filter'], labelFromIndex: number, labelToIndex: number, prevState: LovelyChartState): LovelyChartYRanges {
...
function calculateXAxisScale(plotWidth: number, labelFromIndex: number, labelToIndex: number) {
...
function calculateYAxisScale(plotHeight: number, yMin: number, yMax: number) {
