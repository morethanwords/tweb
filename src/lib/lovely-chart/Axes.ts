...
import {LovelyChartBuildedColors, LovelyChartColorType, LovelyChartPlotSize, LovelyChartProjection, LovelyChartState, StatisticsGraph} from './types';

export function createAxes(
  context: CanvasRenderingContext2D,
  data: StatisticsGraph,
  plotSize: LovelyChartPlotSize,
  colors: LovelyChartBuildedColors,
  font: StatisticsGraph['axesFont'] = AXES_FONT
) {
  function drawXAxis(state: LovelyChartState, projection: LovelyChartProjection) {
  ...
  function drawYAxis(state: LovelyChartState, projection: LovelyChartProjection, secondaryProjection: LovelyChartProjection) {
    const {
    ...
    const colorKey: LovelyChartColorType = secondaryProjection ? `dataset#${data.datasets[0].key}` : undefined;
    ...
    if(secondaryProjection) {
      ...
      const secondaryColorKey: LovelyChartColorType = `dataset#${data.datasets[data.datasets.length - 1].key}`;
  ...
  function _drawYAxisScaled(
    ...
    yMax: number,
    opacity = 1,
    colorKey: LovelyChartColorType = null,
    isSecondary = false
  ) {
  ...
  function _drawYAxisPercents(projection: LovelyChartProjection) {
