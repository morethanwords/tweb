...
export function createTools(container: HTMLElement, data: StatisticsGraph, filterCallback: any) {
  ...
  function _setupLayout() {
    ...
    data.datasets.forEach(({key, name}) => {
      const control = createElement('a') as HTMLAnchorElement;
      const color = data.colors[key];
      control.style.setProperty('--dataset-color', color);
      control.href = '#';
      control.dataset.key = key;
      control.className = `lovely-chart--button lovely-chart--state-checked`;
  ...
  function _updateFilter(button?: HTMLElement, isLongPress: boolean = false) {
