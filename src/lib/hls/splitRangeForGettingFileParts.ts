...
export function splitRangeForGettingFiles(lowerBound: number, upperBound: number) {
  ...
  const limitForFirstPart = 2 ** Math.ceil(Math.log2(diff)); // limit must be power of 2
  ...
  if(currentOffset < upperBound) {
    const diff = upperBound - currentOffset;
    const limitForLastPart = Math.max(2 ** Math.ceil(Math.log2(diff)), STREAM_CHUNK_MULTIPLIER);
    ranges.push({
      offset: currentOffset,
...
// Must be run in a window client to see logs
;(() => {
  const log = logger('test-splitRangeForGettingFiles');
  const cases = [
    [0, 4], // [offset, limit]
    [0, 4095],
    [0, 4096],
  ...
  for(const c of cases) {
    const start = c[0];
    const end = c[0] + c[1] - 1;
    log('start, limit, end, splitRangeForGettingFiles(start, end)',
      start, c[1], end,
      splitRangeForGettingFiles(start, end).ranges
      .map(c => ({...c, offset_div_4096: c.offset / 4096, limit_div_4096: c.limit / 4096, log2_limit: Math.log2(c.limit)}))
    )
  }
})();
